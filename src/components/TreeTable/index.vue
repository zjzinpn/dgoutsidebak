        ...
        <slot :scope="scope" :name="item.key" >
          <template v-if="item.expand">
            ...
            <span v-if="showSperadIcon(scope.row)" class="tree-ctrl" @click="toggleExpanded(scope.$index)">
              ...
              <i v-if="!scope.row._expand" class="el-icon-caret-right" style="color: #606266;font-size: 18px" />
              <i v-else class="el-icon-caret-bottom" style="color: #606266;font-size: 18px" />
              <svg-icon v-if="!scope.row._expand" icon-class="my-folder-close" style="color: #606266;font-size: 18px" />
              <svg-icon v-else icon-class="my-folder-open" style="color: #606266;font-size: 18px" />
            </span>
            <span v-else>
              <!-- <svg-icon icon-class="my-file" style="color: #606266;font-size: 18px" /> -->
               <svg-icon icon-class="my-file" style="color: blue;font-size: 18px" />
            </span>
          </template>
          <template v-if="item.checkbox">
           ...
           <template >
              <span v-if="item.label=='指引标题'" style="color: blue;">
                <a v-if="item.label=='指引标题'" style="text-decoration: underline;" @click="downloadFile(scope.row[item.key].split('|||')[0],scope.row[item.key].split('|||')[1])" >{{scope.row[item.key]|split1}}</a>
             </span>
           <span v-else-if="item.label=='发布时间'" style="color: blue;">
          {{ scope.row[item.key]|formatDate}}
           </span>
           <span v-else >
          {{ scope.row[item.key]}}
           </span>
...
export default {
  ...
  methods: {
    downloadFile(id,title){
        this.downloadQuery.id= id
      this.loading = true
      this.loadingText = '正在导出模板...'
     axios.post('/api/antiepidemic/downloadById', this.downloadQuery,{responseType: 'arraybuffer'}).then(res => {
        this.loading = false
      //  console.log(res.headers['content-disposition'])
     //   let fileName = res.headers['content-disposition'].split('fileName=')[1];
        let fileName = title.replace(/\<span style=\"color:red\"\>/g,'').replace(/\<\/span\>/g,'')
